    ...
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 overflow-y-auto h-screen bg-gray-50 custom-scrollbar">
            <header class="bg-white border-b border-gray-100 px-8 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm">
                ...
                <div class="p-2"> @if (session('status'))
            <div 
                x-data="{ show: true }" 
                x-show="show" 
                ...
                class="mb-2 flex items-center justify-between bg-green-50 border-l-2 border-green-500 p-2 rounded shadow-sm max-w-xs ml-auto"
            >
                <div class="flex items-center">
                    <span class="text-green-700 text-xs font-semibold">{{ session('status') }}</span> 
                    </div>
                
                <button @click="show = false" class="ml-4 text-green-500 hover:text-green-700 transition">
                ...
                <div class="max-w-6xl mx-auto space-y-8">   
                    <div class="lg:hidden mb-6 relative">
                        <span class="absolute inset-y-0 left-0 pl-3 flex items-center text-gray-400">
                            ...
                            class="w-full pl-10 pr-4 py-3 border border-gray-200 rounded-xl focus:ring-red-500 focus:border-red-500 outline-none shadow-sm">
                        </div>
            ...
            <div class="relative">
                ...
                <input type="text" x-model="search" placeholder="Search albums..." 
                    class="pl-10 pr-4 py-2 border border-gray-200 rounded-xl text-sm focus:ring-2 focus:ring-red-500 outline-none transition w-80 shadow-sm">
                    </div>
                            <p class="text-xs text-gray-400 font-medium" x-show="search.length > 0">
                            ...
                            <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100"
                                ...
                                <div class="flex items-center justify-between mb-6 border-b border-gray-50 pb-4">
                                    ...
                                    <form action="{{ route('slideshow.destroy-album') }}" method="POST" 
                                        ...
                                        <button type="submit" class="p-1.5 text-red-500 hover:bg-red-50 border border-red-100 rounded-lg transition active:scale-95 flex items-center justify-center" title="Move to Trash">
                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                    </svg>
                                </button>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                                    @foreach($groupedSlides as $slide)
                                        <div class="bg-gray-50 rounded-xl border border-gray-100 overflow-hidden group hover:shadow-md transition">
                                            ...
                                            <form action="{{ route('slideshow.destroy-album', $slide->id) }}" method="POST" onsubmit="return confirm('Move this photo to trash?')">
                                                    @csrf 
                                                    @method('DELETE')
                                                    <button type="submit" class="p-1.5 text-red-500 hover:bg-red-50 border border-red-100 rounded-lg transition active:scale-95" title="Delete Photo">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
                                                        </svg>
                                                    </button>
